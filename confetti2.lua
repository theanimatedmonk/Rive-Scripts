...
-- fireTrigger: when fired, burst spawns immediately.
...
local function updateColors(self: Confetti2)
  self.colors = {}
  if self.color1 then
...
local function spawnConfettiBurst(self: Confetti2)
...
local function onFireTrigger(self: Confetti2)
...
function init(self: Confetti2, context: Context): boolean
  self.particles = {}
  self.colors = {}
  updateColors(self)
  return true
...
function advance(self: Confetti2, dt: number): boolean
  updateColors(self)
...
function update(self: Confetti2)
  updateColors(self)
end

function draw(self: Confetti2, renderer: Renderer)
  for _, p in ipairs(self.particles) do
    if
      p.y > -MARGIN
...
return function(): Node<Confetti2>
  return {
