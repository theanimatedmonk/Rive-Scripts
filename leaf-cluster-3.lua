-- Leaf cluster 3: same setup as leaf-cluster, different position and orientation.
-- Inputs: flowerDensity, leafDensity 0–5; scale 1–5; windSpeed 0–5; FlowerArtboard, LeafArtboard.

local CANVAS_W = 412
local CANVAS_H = 600
local CLUSTER_CX = 308
local CLUSTER_CY = 360
local CLUSTER_RX = 100
local CLUSTER_RY = 95
local CLUSTER_ROTATION = -0.48
...
local function densityNorm05(self: LeafCluster3, input: string): number
  local v = (input == "flower" and self.flowerDensity or self.leafDensity) or 0
  return clamp(v, 0, 5) / 5
end

local function scaleNorm(self: LeafCluster3): number
  return clamp(self.scale or 3, 1, 5) / 5
end

local function windNorm(self: LeafCluster3): number
...
local function ensureLeafInstance(self: LeafCluster3): boolean
...
local function ensureFlowerInstance(self: LeafCluster3): boolean
...
local function buildLeaves(self: LeafCluster3)
  self.leaves = {}
...
function init(self: LeafCluster3, context: Context): boolean
  self.time = 0
  buildLeaves(self)
  return true
end

function advance(self: LeafCluster3, dt: number): boolean
...
function draw(self: LeafCluster3, renderer: Renderer)
...
return function(): Node<LeafCluster3>
  return {
    flowerDensity = 3,
    leafDensity = 3,
    scale = 3,
