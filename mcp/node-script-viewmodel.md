...
File: [shaders](https://editor.rive.app/file/shaders/2565829) — `Card3D` node drawing a cloned **Mercari** face.

A nested artboard on the same host can be pointed at `Card.propertyOfMercari` in the inspector and will play `idle` / `lookDown` / `twitch`. A scripted drawable that `face:instance()`s the same artboard will **not** pick that up automatically. This is how to wire it so both stay in sync, and so a different face + VM can be swapped later.
...
**Practical node inputs** (no code change to retarget a new face):

| Input | Role |
| --- | --- |
| `face` | `Input<Artboard<nil>>` — any artboard |
| `viewModelProperty` | Nested VM name on the host artboard’s VM (`propertyOfMercari`) |
| `properties` | Comma-separated fields to relay (`idle,lookDown,twitch`) |

Read the source VM from the host, not from a node VM input:

```lua
local card = ctx:rootViewModel() -- or ctx:viewModel() / dataContext parents
...
Relay every listed field the same way: triggers `fire()`, numbers/bools/strings/colors/enums copy `.value`. Bind **triggers first**. Calling `getNumber('twitch')` and then `.value` throws `attempt to index userdata with 'value'`.
...
## Swap in another artboard tomorrow
